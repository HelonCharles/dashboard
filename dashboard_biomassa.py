...
try:
    ...
    with st.sidebar:
        ...
        ano = st.selectbox("Selecione o ano", ["2022", "2023", "2024", "2025"], index=3)
        col_exp = f"exploracao_{ano}" 
        col_saldo = f"saldo_{ano}"
        
        st.markdown("---")
        lista_talhoes = sorted(data['fid'].unique().tolist(), key=lambda x: int(x))
        talhao_selecionado = st.selectbox("Focar em Talhão", ["Visão Geral"] + lista_talhoes)

    # 3. KPIs de Topo
    total_original = data['mudas_2020'].sum()
    saldo_atual = data[col_saldo].sum()
    progresso_geral = (1 - (saldo_atual / total_original)) * 100

    c1, c2, c3 = st.columns(3)
    c1.metric("Estoque Inicial (2020)", f"{total_original:,.0f}".replace(",", "."))
    c2.metric(f"Saldo {ano}", f"{saldo_atual:,.0f}".replace(",", "."), delta=f"-{total_original - saldo_atual:,.0f}", delta_color="inverse")
    c3.metric("Progresso Total", f"{progresso_geral:.1f}%")

    # 4. Mapa e Informações Rápidas
    st.markdown("---")
    ...
    with col_info:
        if talhao_selecionado != "Visão Geral":
            ...
            st.write(f"**Consumo Atual:** {t_data[col_exp]:.1f}%")
            st.progress(min(float(t_data[col_exp])/100, 1.0))
            st.write(f"**Saldo Remanescente:** {t_data[col_saldo]:,.0f}")
        else:
            st.info("Selecione um talhão no menu lateral para ver detalhes.")

    with col_mapa:
        m = leafmap.Map(center=[2.82, -60.67], zoom=12, google_map="SATELLITE")
        def style_fn(f):
            v = f['properties'][col_exp]
            return {
                'fillOpacity': 0.7, 'weight': 1, 'color': 'white',
                'fillColor': '#ff0000' if v >= 70 else '#ffff00' if v >= 30 else '#00ff00'
            }
        m.add_gdf(data, layer_name="Status", style_function=style_fn)
        
        if talhao_selecionado != "Visão Geral":
            foco = data[data['fid'] == talhao_selecionado]
            m.add_gdf(foco, layer_name="Destaque", style={'color': 'yellow', 'weight': 4, 'fillOpacity': 0})
            centroid = foco.geometry.centroid.iloc[0]
            folium.Marker([centroid.y, centroid.x], popup=f"ID {talhao_selecionado}").add_to(m)
        m.to_streamlit(height=500)

    # 5. Relatório Detalhado (Tabela)
    st.markdown("---")
    df_tabela = data[['fid', 'mudas_2020', col_saldo, col_exp]].copy()
    
    if talhao_selecionado != "Visão Geral":
        df_tabela['is_sel'] = df_tabela['fid'].apply(lambda x: 1 if str(x) == str(talhao_selecionado) else 0)
        df_tabela = df_tabela.sort_values(['is_sel', col_exp], ascending=[False, False]).drop(columns='is_sel')
    else:
    ...
    st.dataframe(
        df_tabela.style.apply(color_rows, axis=1).format({'mudas_2020': '{:,.0f}', col_saldo: '{:,.0f}', col_exp: '{:.1f}%'}),
        column_config={"fid": "ID", "mudas_2020": "Estoque", col_saldo: "Saldo", col_exp: "% Consumo"},
        use_container_width=True, hide_index=True, height=350
    )

    # 6. Gráficos (Ranking e Linha do Tempo)
    st.markdown("---")
    with st.expander("📊 Análise Estatística e Histórica", expanded=True):
        
        # Ranking de Barras
        df_g = df_tabela.copy()
        df_g['Cor'] = df_g['fid'].apply(lambda x: 'Selecionado' if str(x) == str(talhao_selecionado) else 'Outros')
        
        fig_bar = px.bar(
            df_g, x='fid', y=col_exp, color='Cor',
            color_discrete_map={'Selecionado': '#FAFF00', 'Outros': '#A0A0A0'},
            category_orders={"fid": df_g['fid'].tolist()}
        )
        fig_bar.update_layout(showlegend=False, height=400, plot_bgcolor='rgba(0,0,0,0)', xaxis_title="ID do Talhão")
        st.plotly_chart(fig_bar, use_container_width=True)

        # SEÇÃO DO GRÁFICO DE LINHAS (HISTÓRICO)
        if talhao_selecionado != "Visão Geral":
            st.markdown("---")
            st.subheader(f"📈 Evolução Temporal: Talhão {talhao_selecionado}")
            
            anos_lista = ["2022", "2023", "2024", "2025"]
            t_row = data[data['fid'] == talhao_selecionado].iloc[0]
            df_hist = pd.DataFrame([
                {"Ano": a, "Consumo": t_row[f"exploracao_{a}"]} for a in anos_lista
            ])
            
            fig_line = px.line(df_hist, x="Ano", y="Consumo", markers=True, text=[f"{v:.1f}%" for v in df_hist["Consumo"]])
            fig_line.update_traces(line_color='#FAFF00', line_width=4, marker=dict(size=12, color="black", symbol="diamond"))
            fig_line.update_layout(height=350, yaxis_range=[0, 105], yaxis_title="% Consumo Acumulado", plot_bgcolor='rgba(0,0,0,0)')
            st.plotly_chart(fig_line, use_container_width=True)

except Exception as e:
    st.error(f"⚠️ Erro ao carregar dashboard: {e}")
    st.exception(e)streamlit as st
import leafmap.foliumap as leafmap
import geopandas as gpd
from streamlit_folium import st_folium
import pandas as pd
